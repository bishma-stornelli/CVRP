...
public class MoveSingle implements Move {
    ...
    public void applyMove(Solution solution)
        ...
        Route origin_Route =
                solution.getRoute(solution.getRouteNumber(this.customer));
        Route target_Route = solution.getRoute(this.targetRoute);
        int duration =
                solution.getDuration();
        duration +=
                origin_Route.remove(customerPosition, solution.getInstance());
        if (origin_Route.equals(target_Route) && this.targetPosition
                > customerPosition) {
            this.targetPosition = this.targetPosition - 1;
        }
        duration += target_Route.add(this.customer, this.targetPosition,
        ...
        for (int index = customerPosition; index < origin_Route.size() - 1; ++index) {
            solution.setCustomerPosition(origin_Route.getCustomerAt(index),
                    index);
        }
        for (int index = this.targetPosition + 1; index
                < target_Route.size() - 1; ++index) {
            solution.setCustomerPosition(target_Route.getCustomerAt(index),
                    index);
        }
    ...
    public int applyMoveDuration(Solution s)
        ...
        int customerPosition =
                s.getCustomerPosition(this.customer);
        Route rA =
                s.getRoute(s.getRouteNumber(this.customer));
        Route rB =
                s.getRoute(this.targetRoute);
        int origin_RouteDuration =
                rA.getDuration();
        int target_RouteDuration = rB.getDuration();
        int previous = rA.getRoute().get(customerPosition - 1);
        int next =
                rA.getRoute().get(customerPosition + 1);
        int changeOnDuration =
                i.getDistance(previous, next) - i.getDistance(previous,
                this.customer) - i.getDistance(this.customer, next)
                - i.getDropTime();

        int totalChangeOnDuration = changeOnDuration;

        if (origin_RouteDuration + changeOnDuration
                > i.getMaximumRouteTime()) {
        ...
        next =
                rB.getRoute().get(targetPositionAux);
        ...
        changeOnDuration = i.getDistance(previous, this.customer)
                + i.getDistance(this.customer, next) + i.getDropTime()
                - i.getDistance(previous, next);

        totalChangeOnDuration += changeOnDuration;

        if (target_RouteDuration + changeOnDuration
                > i.getMaximumRouteTime()) {
            throw new MaxDurationExceededException();
        }
