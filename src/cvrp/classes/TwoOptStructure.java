 ...
 * @author tamerdark
 */
public class TwoOptStructure implements NeighborhoodStructure {
        ...
        for(int i = 0 ; i < s.getInstance().getCustomersNumber() ; ++i){
            if(s.getRoute(i).size() > 4){
                try {
                    ...
                    Logger.getLogger(TwoOptStructure.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public Neighbor generateNeighbor(Solution s, List<Tabu> tabuList,
        ...
        Move m = new TwoOptMove(route, edge1, edge2);
